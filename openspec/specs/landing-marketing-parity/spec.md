# landing-marketing-parity Specification

## Purpose

Marketing and product-description copy (landing page, pricing, SEO, Help, Privacy, Terms, and relevant Settings text) must match what the shipped application actually does. This spec defines requirements for claim inventories, tier-qualified language, substantiated trust claims, and parity checks when copy changes.
...
- **WHEN** a reviewer compares the live or staged landing page to the inventory in this spec (Appendix A)
...
### Requirement: Parity check before shipping marketing copy changes

Before merging a change that alters landing marketing copy, pricing bullets, structured data, or materially changes Help/Privacy/Terms/Settings text that describes product capabilities, the author SHALL complete a parity pass against this spec’s inventory and scenarios, updating Appendix A and requirements when the canonical promise changes.

#### Scenario: Landing or SEO update merged
...
#### Scenario: Help or legal product-description update merged

- **WHEN** a pull request modifies Help FAQs, Privacy Policy, Terms of Service, or Settings copy that describes what users can do in the product
- **THEN** the change is checked against the “Help, legal, and settings copy” requirement and Appendix A (or a short delta note) is updated if new claims appear
...
Reference implementation: `src/lib/abilities.ts` (`CREDIT_COSTS`, `STORAGE_QUOTAS`, `createAbilityFor`). Last reviewed: 2026-03-21 (codebase review + build). Update this appendix when landing, pricing, or related product copy changes.
...
| --- | --- | --- |
| Marketing claim inventory | **Pass** | Appendix A is the living inventory in `openspec/specs/landing-marketing-parity/spec.md`. |
...
| Parity check before shipping | **Process** | Record in PR template when team adds one. |
