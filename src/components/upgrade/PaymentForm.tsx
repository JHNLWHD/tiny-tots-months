...
export const PaymentForm: React.FC = () => {
	...
	const handleUpgrade = async () => {
		try {
			...
			setError(
				error?.message || "There was a problem processing your upgrade request",
			);
	...
	return (
		<div className="space-y-6">
			...
			<div className="space-y-3">
				...
				{!preview ? (
					<button
						className={`w-full border-2 border-dashed ${error ? "border-destructive" : "border-gray-300"} rounded-lg p-6 text-center cursor-pointer transition-colors ${isLoading ? "opacity-50 pointer-events-none" : "hover:border-baby-purple hover:bg-gray-50"}`}
						onClick={() =>
							!isLoading && document.getElementById("receipt-upload")?.click()
						}
						type="button"
					>
